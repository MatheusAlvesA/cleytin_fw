/**
 * Boa parte do código que controla a tela de LCD foi baseado no código de exemplo da própria Espressif 
 * https://github.com/espressif/esp-idf/tree/master/examples/peripherals/spi_master/lcd
 */

#ifndef CLEYTIN_TFTLCD_API
#define CLEYTIN_TFTLCD_API
...
typedef struct {
    uint8_t cmd;
    uint8_t data[16]; // Nenhum comando precisa de mais que 16 bytes de parâmetros
    uint8_t sizeData;
} lcd_init_cmd_t;

// As sequencias de inicialização ficam na memória DRAM para garantir que o SPI vai consguir acessar via DMA
